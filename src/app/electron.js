...
function createWindow() {
  ...
  if (windowState.manage) {
    try { windowState.manage(win) } catch {}
  }
...
function registerIpcHandlers() {
  ...
  ipcMain.on('remove-file', (_event, data) => {
    try { fs.rmSync(data.path, { force: true }) } catch {}
  })

  ipcMain.on('remove-files', (_event, data) => {
    if (data?.paths) {
      data.paths.forEach((item) => {
        try { fs.rmSync(item.src, { force: true }) } catch {}
      })
    }
  })
    ...
    dialog
      ...
      .then((result) => {
        ...
        try {
          ...
          event.sender.send('open-folder-dialog-reply', { folderPath, tree: null })
        }
      })
      .catch((err) => {
